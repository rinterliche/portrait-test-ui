...
function AppForm() {
  ...
  const from = new Date(formParams.from).toLocaleDateString("en-US")
  const to = new Date(formParams.to).toLocaleDateString("en-US")

  return (
    <div className="mb-10">
      <form className="max-w-md mx-auto flex gap-5" onSubmit={handleSubmit}>
      ...
      <p class="mt-2 text-xs text-gray-500 dark:text-gray-400 text-center">From {from} to {to}</p>
    </div>
